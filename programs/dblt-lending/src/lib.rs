...
pub mod dblt_lending {
    ...
    /// Submits a review (rating and comment) for a completed loan.
    /// The reviewer must be a participant (lender or borrower) of the loan.
    pub fn submit_review(ctx: Context<SubmitReview>, rating: u8, comment: String) -> Result<()> {
        handlers::submit_review(ctx, rating, comment)
    }
}