...
const statusStyle = computed(() => {
  ...
  if (BigInt(props.repaid) >= BigInt(props.amount)) return 'border-emerald/40 bg-emerald/10 text-emerald'
...
const statusLabel = computed(() => {
  ...
  if (BigInt(props.repaid) >= BigInt(props.amount)) return 'Repaid'
  return                                        'Defaulted'
})
  ...
  <div class="glass-panel rounded p-4 flex flex-col gap-3 cursor-pointer" @click="emit('view')">
    ...
    <div>
      <p class="font-mono text-xl font-bold text-white">
        {{ toSol(BigInt(amount)) }} <span class="text-white/50 text-sm font-normal">{{ currency }}</span>
      </p>
    ...
    <div class="flex items-center justify-between gap-3 text-[11px] text-muted flex-wrap">
      <div class="flex items-center gap-3">
        <span>APY <span class="font-mono font-bold text-white">{{ (Number(apy) / 100).toFixed(2) }}%</span></span>
