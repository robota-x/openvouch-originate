...
// Used as a mock for demo profiles that have no on-chain computed score yet.
  ...
  try {
    if (c.get('config').fixturesEnabled) {
      const profile = fixtureProfiles[address] ?? {
        ...
        trustScore:  computeTrustScore(address, 0),
        attestations: [],
        loans:        [],
      }
    ...
    const verifiedCount = attestationRows.filter(a => a.verified).length
    return c.json({
      address:    row.address,
      nickname:   row.nickname ?? row.address.slice(0, 8),
      trustScore: row.trustScore ?? computeTrustScore(row.address, verifiedCount),
    ...
    console.error(`[profiles/:address] Failure: address=${address}`, e)
...
profileRoutes.patch('/:address', authenticate, async (c) => {
  ...
  try {
    ...
    console.error(`[profiles/:address PATCH] Failure: address=${address} userAddress=${user.address}`, e)
    return c.json({ error: 'internal_server_error' }, 500)
  }
})
